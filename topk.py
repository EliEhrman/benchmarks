...
v_res = tf.Variable(tf.zeros([c_num,c_num_ks], dtype=tf.int32), name='v_res')
op_data_set = tf.assign(v_data, tf.random_normal(shape=[c_num,c_len], mean=0.0, stddev=0.1, dtype=tf.float32), name='op_data_set')
_, t_idxs = tf.nn.top_k(v_data, k=c_num_ks, sorted=False)
op_res_set = tf.assign(v_res, t_idxs, name='op_res_set')
